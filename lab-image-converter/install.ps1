...
# Also check Hebrew desktop name
...
# Try known shell folder from registry
try {
    ...
    $candidates = @($regDesktop) + $candidates
} catch {}

foreach ($candidate in $candidates) {
    if ($candidate -and $candidate -ne "" -and (Test-Path $candidate)) {
...
if ($DesktopPath) {
    ...
    Write-Host "  Desktop folder not found. Double-click START.cmd to launch." -ForegroundColor Yellow
}
...
Write-Host "    Option 2: Double-click START.cmd in this folder"
Write-Host ""
Write-Host "  How to stop:"
Write-Host "    Close the server window, or double-click STOP.cmd"
Write-Host ""
