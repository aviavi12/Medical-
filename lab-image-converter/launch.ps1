...
if (-not (Test-Path ".venv\Scripts\python.exe")) {
    Write-Host "ERROR: Virtual environment not found. Run INSTALL.cmd first." -ForegroundColor Red
...
$params = @{ FilePath = ".venv\Scripts\python.exe"; ArgumentList = "-m uvicorn app.main:app --host 127.0.0.1 --port $Port"; WorkingDirectory = $ScriptDir; PassThru = $true }
$job = Start-Process @params
...
if ($ready) {
    ...
    Write-Host "To stop: close the server window or double-click STOP.cmd" -ForegroundColor Yellow
} else {
